...
if (savedData.dayLastUpdated == undefined)
...
let onNewDay = function () {
  ...
  if (savedData.mostActiveDay.cals < calData.activeCaloriesBurned) {
    // new most active day
    savedData.mostActiveDay = {
      cals: calData.activeCaloriesBurned,
      date: Math.floor(new Date(Date.now() - 86400000).getTime() / 1000)
    };
  }
  if (savedData.mostCalorieDay.cals < calData.totalCaloriesBurned) {
    // new most calories day
    savedData.mostCalorieDay = {
      cals: calData.totalCaloriesBurned,
      date: Math.floor(new Date(Date.now() - 86400000).getTime() / 1000)
    };
  }
...
Bangle.on("health", function (hd) {
  ...
  let cd = calModule.calcCalories(
    Object.assign(hd, { duration: 10 }),
    myProfile
  );
  if (!cd) return;
  ...
  if (
    calData.activeCaloriesBurned >= settings.calGoal &&
    !savedData.goalShownToday &&
    settings.showGoalReached
  ) {
